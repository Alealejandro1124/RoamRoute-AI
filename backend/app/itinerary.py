...
import random
from .schemas import Itinerary as ItinerarySchema, ScheduleItem
...
# Suggestions for each activity type (renamed from "alternative_suggestions")
...
async def generate_auto_itinerary(user_id: int, db: Session = Depends(get_db)):
    """
    Automatically generates a single itinerary for the user,
    containing one suggestion for each time window:
    Breakfast, Early Morning Activity, Lunch, Afternoon Activity,
    Dinner, and Evening Activity.
    Each suggestion is chosen at random from the available list.
    """
    try:
        ...
        # Build an itinerary with a random item per time window
        schedule = [
...
async def get_itinerary(user_id: int, db: Session = Depends(get_db)):
    """
    Fetches the automatically generated itinerary for a user.
    """
    try:
...
async def update_activity_feedback(
    ...
    """
    Provides feedback for an activity in the itinerary associated with this user_id.
    If thumbs up (rating == 2), returns a thank you message.
    If thumbs down (rating == 1), replaces the activity with a random new suggestion
    (excluding the current suggestion).
    """
    ...
    for idx, item in enumerate(itinerary.schedule):
        if item.get("activity") == activity:
            if rating == 2:
                ...
                # Find suggestions for the same activity type
                current_suggestion = item.get("suggestion")
                possible_suggestions = suggestions.get(activity, [])

                # Filter out the current suggestion so we don't repeat it
